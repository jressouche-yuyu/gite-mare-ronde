// Génère les fichiers de contenu des gîtes (collection Tina) depuis les données actuelles.
import { writeFileSync, mkdirSync } from 'node:fs';

const pics = (slug, n) => Array.from({ length: n }, (_, i) => `/uploads/${slug}/${String(i + 1).padStart(2, '0')}.jpg`);
const AIRBNB = {
...
const gites = [
  ...
  },
  {
    ...
    airbnb: AIRBNB.escapade,
  },
];

mkdirSync('src/content/gites', { recursive: true });
for (const g of gites) {
  writeFileSync(`src/content/gites/${g.slug}.json`, JSON.stringify(g, null, 2) + '\n');
  console.log('écrit', g.slug, '·', g.photos.length, 'photos');
}
